...
export function DynamicTags(props: DynamicTagsProps) {
  ...
  onMount(() => {
    if (props.showAll || !containerRef) {
      setVisibleCount(props.tags.length)
      return
    }

    const update = () => {
      if (containerRef)
        setVisibleCount(estimateVisibleTags(props.tags, containerRef.offsetWidth))
    }
  ...
  const visibleTags = createMemo(() => props.showAll ? props.tags : props.tags.slice(0, visibleCount()))
  const hiddenTags = createMemo(() => props.showAll ? [] : props.tags.slice(visibleCount()))

  return (
    <div ref={containerRef} class={`relative w-full ${props.class ?? ''}`}>
      <div class={`flex gap-2 ${props.showAll ? 'flex-wrap' : 'items-center overflow-hidden'}`}>
